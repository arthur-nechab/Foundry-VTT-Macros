// Permet de choisir une table et de sélectionner le nombre de jet de dés
...
let content = `<form><div style="display: inline-block; width: 100%; margin-bottom: 10px">
  <label for="output-tableKey" style="vertical-align: top; margin-right: 10px;">Nom :</label><br /><select name="output-tableKey" id="output-tableKey">`

tables.forEach(table => {
 content += `<option value='${table.key}'>${table.name}</option>`;
});

content += `</select><br /><label for="output-numberRolls">Jet de dés : </label><br /><input name="output-numberRolls" value="1" /><br />`
